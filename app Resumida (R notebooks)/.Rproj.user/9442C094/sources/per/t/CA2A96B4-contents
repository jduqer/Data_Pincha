...
server <- function(input, output, session) {
  ...
  apellidos <- reactive({
    columnas <- input$jugador1
    return(columnas)

  })

  output$value1 <- renderDataTable({ df <-  soccer %>% 
      select(Minuto.video,segundos,Apellido,Jugador.2,remate1(),pase1(),centros1(),duelos1(),faltas1(),recuperacion1(),parado1(),
             perdida1(),arquero1(),inicial_x,inicial_y,final_x,final_y, partido) %>% 
      filter(Apellido %in% apellidos()) 
  if(ncol(df) > 3){
    salida <- data.frame(Minuto.video = as.numeric(),
                         Apellido = as.character(),
  ...
  df <-  soccer %>% 
    select(Minuto.video,Apellido,Jugador.2,remate1(),pase1(),centros1(),duelos1(),faltas1(),recuperacion1(),parado1(),
           perdida1(),arquero1(),inicial_x,inicial_y,final_x,final_y, partido, Evento, Subevento ) %>% 
    filter(Apellido %in% apellidos()) 
  if(ncol(df) > 3){
  ...
  ggplotly(ggplot(soccer_estandar_destino) +
      ...
      geom_segment(aes(x = inicial_x, y = inicial_y, xend = final_x, yend = final_y, colour=soccer_estandar_destino$Apellido),
                   arrow = arrow(length = unit(0.25, "cm"),
                                 type = "closed")) +
      theme_pitch() +
      geom_point(aes(x = inicial_x, y = inicial_y, fill = soccer_estandar_destino$Apellido),
                 colour = "black",
                 pch = 21,
                  size = 5)# +
        ...
        #                  y = -3,
        #                  xend = 60,
        #                  yend = -3),arrow = arrow(length = unit(0.2, "cm")))
      # direction_label(
      #   x_label = 50,
      #   y_label = -3,
      #   label_length = 20,
      #   colour = "dimgray") 
      # annotate("text", x=soccer_estandar_destino$inicial_x,y=soccer_estandar_destino$inicial_y, label=soccer_estandar_destino$Apellido, color="white", size=3.5)
)
  })
  ...
  output$value2 <- renderDataTable({ df <-  soccer %>% 
    select(Minuto.video,segundos,Apellido,Jugador.2,remate2(),pase2(),centros2(),duelos2(),faltas2(),recuperacion2(),parado2(),
  ...
  output$plot2 <- renderPlotly({
    ...
    df <-  soccer %>% 
      select(Minuto.video,Apellido,Jugador.2,remate2(),pase2(),centros2(),duelos2(),faltas2(),recuperacion2(),parado2(),
             perdida2(),arquero2(),inicial_x,inicial_y,final_x,final_y, partido) %>% 
      filter(partido %in% input$partido2) 
    if(ncol(df) > 3){
      soccer_estandar_destino <- data.frame(Minuto.video = as.numeric(),
    ...
    ggplotly( ggplot(soccer_estandar_destino) +
      annotate_pitch(colour = "white",
                     ...
                     limits = TRUE,
                     dimensions = pitch_opta) +
      geom_segment(aes(x = inicial_x, y = inicial_y, xend = final_x, yend = final_y, colour=Apellido),
                   arrow = arrow(length = unit(0.25, "cm"),
                                 type = "closed")) +
      theme_pitch() +
      geom_point(aes(x = inicial_x, y = inicial_y, fill = Apellido),
                 colour = "black",
                 pch = 21,
                 size = 5)  +
      direction_label(
  ...
  output$value3 <- renderDataTable({ df <-  soccer %>% 
    select(Minuto.video,Apellido,Jugador.2,remate3(),pase3(),centros3(),duelos3(),faltas3(),recuperacion3(),parado3(),
           perdida3(),arquero3(),inicial_x,inicial_y,final_x,final_y, partido) %>% 
    filter(partido %in% input$partido3, Apellido %in% apellidos3()) 
  if(ncol(df) > 3){
    salida <- data.frame(Minuto.video = as.numeric(),
                         Apellido = as.character(),
                         Jugador.2 = as.numeric())
  ...
  output$plot3 <- renderPlotly({
    ...
    df <-  soccer %>% 
      ...
      filter(partido %in% input$partido3, Apellido %in% input$jugador3) 
    if(ncol(df) > 3){
      soccer_estandar_destino <- data.frame(Minuto.video = as.numeric(),
